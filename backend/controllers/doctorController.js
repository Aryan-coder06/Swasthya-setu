...
import { listPrescriptionReports } from "../models/prescriptions.js";
import {
  generateMeetingLink,
  saveMeetingLink,
  getDoctorProfile,
...
const listDoctorDirectory = async (req, res) => {
 ...
 const getPrescriptionsByPatientId = async (req, res) => {
  try {
    ...
    if (error) {
      console.error("List prescriptions by patient ID error:", error);
      return res.status(500).json({ error: "Could not load prescriptions." });
    }

    // Normalize medications to always be an array
    ...
    console.error("Get prescriptions by patient ID handler error:", error);
    return res.status(500).json({ error: "Internal Server Error" });
  }
};
